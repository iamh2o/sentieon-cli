...
class Command(ShellNode):
    ...
    def __str__(self) -> str:
        ...
        # ``env`` is execution metadata, not command text.  In particular,
        # Hybrid passes a copy of the inherited environment here, so rendering
        # it would disclose credentials and other process-local values in dry
        # runs, manifests, and executor logs.  Command.run() still passes the
        # unmodified mapping to create_subprocess_exec().
    ...
    def __repr__(self) -> str:
        exec_kwargs = "<redacted>" if self.exec_kwargs else "{}"
        return (
