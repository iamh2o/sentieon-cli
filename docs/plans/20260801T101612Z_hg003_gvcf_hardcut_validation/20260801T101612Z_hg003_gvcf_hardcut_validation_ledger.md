...
| GVCF-001 | Original lane | Run upstream Sentieon CLI `v1.7.0 --gvcf` on the frozen prepared CRAMs | IN_PROGRESS | execution | 1 | Codex | Slurm `2598`, submitted with `--partition i128nvme --comment RnD --cpus-per-task=128`; exact source `1bf377d` | N/A | Configuring on a dedicated i128nvme node |
| GVCF-002 | Accelerated lane | Run released fork `1.7.1i --gvcf` on exactly the same prepared CRAMs and arguments | IN_PROGRESS | execution | 1 | Codex | Slurm `2599`, submitted with `--partition i128nvme --comment RnD --cpus-per-task=128`; annotated tag peels to `e4f0ff8` | N/A | Configuring on a separate i128nvme node in parallel with job 2598 |
...
| HARDCUT-001 | DayOA hardcut | Derive one hard VCF from each gVCF using the exact DayOA `13.0.116` GVCFtyper, sort, and index commands | OPEN | execution | 2 | Codex | Slurm `2600` submitted with `afterok:2598:2599`; exact DayOA tag checkout verified | N/A | Dependency-held until both gVCFs publish successfully |
