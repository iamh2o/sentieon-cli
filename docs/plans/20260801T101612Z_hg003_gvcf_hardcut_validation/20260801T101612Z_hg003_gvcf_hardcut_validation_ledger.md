...
| INV-001 | Inventory | Verify clean exact-tag CLI checkouts, frozen inputs, truth data, tools, scheduler contract, and output-name non-collision | IN_PROGRESS | validation | 0 | Codex | Detached exact-SHA worktrees created under `checkouts/official170` and `checkouts/release171i`; frozen prepared CRAMs and prior native-hard results exist; remaining payload/tool preflight occurs inside each fail-hard Slurm job | N/A | No gVCF result directory exists yet |
| GVCF-001 | Original lane | Run upstream Sentieon CLI `v1.7.0 --gvcf` on the frozen prepared CRAMs | OPEN | execution | 1 | Codex | Pending Slurm job | N/A | Pending |
| GVCF-002 | Accelerated lane | Run released fork `1.7.1i --gvcf` on exactly the same prepared CRAMs and arguments | OPEN | execution | 1 | Codex | Pending Slurm job | N/A | Pending |
...
| HARDCUT-001 | DayOA hardcut | Derive one hard VCF from each gVCF using the exact DayOA `13.0.116` GVCFtyper, sort, and index commands | OPEN | execution | 2 | Codex | Pending hardcut manifests | N/A | Pending |
