...
def test_str_and_repr_redact_exec_environment():
    """Display forms cannot serialize inherited environment data."""
    secret = "arbitrary-secret-value"
    child_env = dict(os.environ)
    child_env["SENSITIVE_TOKEN"] = secret
    env_cmd = Command(
        "samtools",
        "sort",
        exec_kwargs={"env": child_env},
    )
    assert str(env_cmd) == "samtools sort"
    assert "SENSITIVE_TOKEN" not in str(env_cmd)
    assert secret not in str(env_cmd)
    assert "SENSITIVE_TOKEN" not in repr(env_cmd)
    assert secret not in repr(env_cmd)
    assert "exec_kwargs=<redacted>" in repr(env_cmd)
    ...
    both = Command(
        ...
        exec_kwargs={
            "cwd": "/data dir",
            "env": {"SENSITIVE_TOKEN": secret},
        },
    )
    assert str(both) == "(cd '/data dir' && run)"
    assert secret not in str(both)
    assert secret not in repr(both)


@pytest.mark.asyncio
async def test_command_executes_with_unrendered_environment(tmp_path):
    """Redaction is display-only; the subprocess receives the real env."""
    secret = "child-received-this-value"
    child_env = dict(os.environ)
    child_env["TOKEN"] = secret
    ...
    assert secret in output.read_text().splitlines()
